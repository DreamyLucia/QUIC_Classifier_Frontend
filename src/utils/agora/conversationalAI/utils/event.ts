...
export class EventHelper<T> {
  ...
  on<Key extends keyof T>(evt: Key, cb: T[Key]) {
    ...
    console.info(`Subscribed to event: ${String(evt)}`)
    return this
  }
  ...
  off<Key extends keyof T>(evt: Key, cb: T[Key]) {
    ...
    if (cbs) {
      ...
      console.info(`Unsubscribed from event: ${String(evt)}`)
    }
    return this
  }
  ...
  removeAllEventListeners(): void {
    ...
    console.info('Removed all event listeners')
  }
  ...
  emit<Key extends keyof T>(evt: Key, ...args: any[]) {
    ...
    console.info({ args }, `Emitted event: ${String(evt)}`)
    return this
  }
}
